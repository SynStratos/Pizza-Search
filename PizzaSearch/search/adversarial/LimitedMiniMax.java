...
public class LimitedMiniMax {
	int PLAYER = -1;
	int COMPUTER = 1;
	public Object[] minimax(Game game, int player, int maxDepth)
	{
		...
		if(moves.isEmpty() || maxDepth==0) {
			...
			for(Move i : moves) {
				...
				//devo minimizzare il valore nel caso in cui il giocatore selezionato sia l'avversario (umano)
					int score = (Integer)minimax(game, COMPUTER, maxDepth-1)[0];
					if(score < bestScore) {
						bestScore = score;
						move = i;
					}
				...
				//devo massimizzare il valore nel caso in cui il giocatore selezionato sia il computer
					int score = (Integer)minimax(game, PLAYER, maxDepth-1)[0];
					if(score > bestScore) {
						bestScore = score;
						move = i;
					}
