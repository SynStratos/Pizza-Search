...
public class LimitedAlphaBeta {
	int PLAYER = -1;
	int COMPUTER = 1;
	
	public Object[] alphabeta(Game game, int player, int alpha, int beta, int maxdepth)
	{
		...
		if(moves.isEmpty() || maxdepth == 0) {
			...
			for(Move i : moves) {				
				...
				//devo minimizzare il valore nel caso in cui il giocatore selezionato sia l'avversario (umano)
					score = (Integer)alphabeta(game, COMPUTER, alpha, beta, maxdepth-1)[0];
					if(score < beta) {
						beta = score;
						move = i;
					}
				...
				//devo massimizzare il valore nel caso in cui il giocatore selezionato sia il computer
					score = (Integer)alphabeta(game, PLAYER, alpha, beta, maxdepth-1)[0];
					if(score > alpha) {
						alpha = score;
						move = i;
					}
